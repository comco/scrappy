...
trait DataImplicits {
  def tuple(coordinates: Data.Any*): TupleData = {
    TupleData(coordinates: _*)
  }

  def seq(elements: Data.Any*): SeqData = {
    SeqData(elements.head, elements.tail: _*)
  }

  def struct(symbol: Symbol)(features: (Symbol, Data.Any)*)(implicit repo: TypeRepository): StructData = {
    StructData(repo.getStructType(symbol))(features.map {
      case (sym, data) => (sym.name, data)
    }: _*)
  }

  def some(data: Data.Any): SomeData = {
  ...
  implicit class RichSymbol(symbol: Symbol) {
    def of(features: (Symbol, Data.Any)*)(implicit repo: TypeRepository): StructData =
