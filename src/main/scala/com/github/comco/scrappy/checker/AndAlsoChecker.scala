...
case class AndAlsoChecker[Shape <: Shape.Any](
  ...
  override def doCheck(source: Data[Shape]): Result[Shape] = {
    ...
    if (!result1.successful) {
      return Result(false, source.origin, Set(result1), this)
    } else {
      val result2 = checker2.check(source)
      if (result2.successful) {
        return Result(true, source.origin, Set(result1, result2), this)
      } else {
        return Result(false, source.origin, Set(result2), this)
      }
    }
  }
}