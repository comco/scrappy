...
class ApplyPickerSpec extends FlatSpec {
  ...
  it should "pickOriginatedData" in {
    val originated = OriginatedDataDomain.mkOriginatedDataFrom(data, OriginalOrigin(SelfPointer(IntPrimitiveType)))
    val result = incPicker.pickOriginatedData(originated)
    result shouldEqual OriginatedDataDomain.mkOriginatedDataFrom(4, ComputedOrigin(IntPrimitiveType, IntPrimitiveType, Set(SelfPointer(IntPrimitiveType))))
  }
}